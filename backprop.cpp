    ...
    while (this->system->training_view->get_epochs() > 0 && this->system->get_is_training()) {
        ...
        for (int i = 0; i < sharded_data.first->get_rows(); i += common_size_batch) {
            ...
            for (int c = 0; c < temp_layers.size() - 1; c++) {
                ...
                clFinish(queue);
                err = clEnqueueCopyBuffer(queue, cl_result_vector, pre_activations.back(), 0, 0, size_R, 0, nullptr, nullptr);
                OCL_SAFE_CALL(err);
                clFinish(queue);
            ...
            if (activations_layers.back() == Activation::RELU) {
                ...
                clFinish(queue);
                pre_acts_mod.push_back(clCreateBuffer(context, CL_MEM_READ_ONLY, size * sizeof(float), nullptr, &err));
                clFinish(queue);
                ...
                clFinish(queue);
                acts_mod.push_back(clCreateBuffer(context, CL_MEM_READ_ONLY, size * sizeof(float), nullptr, &err));
                clFinish(queue);
            ...
            for (int l = temp_layers.size() - 2; l >= 0; l--) {
                if (activations_layers[l] == Activation::RELU) {
                    ...
                    pre_acts_mod.push_back(clCreateBuffer(context, CL_MEM_WRITE_ONLY, size * sizeof(float), nullptr, &err));
                    clFinish(queue);

                    OCL_SAFE_CALL(clSetKernelArg(system->kernel_relu_deriv_simple, 0, sizeof(cl_mem), &pre_activations[l]));
                    OCL_SAFE_CALL(clSetKernelArg(system->kernel_relu_deriv_simple, 1, sizeof(cl_mem), &pre_acts_mod.back()));
                    OCL_SAFE_CALL(clSetKernelArg(system->kernel_relu_deriv_simple, 2, sizeof(cl_int), &size));
            ...
            for (int l = temp_layers.size() - 2; l >= 0; l--) {
                if (activations_layers[l] == Activation::SIGMOID) {
                    int size = size_batch * temp_layers[l + 1]->num_neuros;
                    acts_mod.push_back(clCreateBuffer(context, CL_MEM_WRITE_ONLY, size * sizeof(float), nullptr, &err));

                    OCL_SAFE_CALL(clSetKernelArg(system->kernel_sigmoid_deriv_simple, 0, sizeof(cl_mem), &activations[l]));
                    OCL_SAFE_CALL(clSetKernelArg(system->kernel_sigmoid_deriv_simple, 1, sizeof(cl_mem), &acts_mod.back()));
                    OCL_SAFE_CALL(clSetKernelArg(system->kernel_sigmoid_deriv_simple, 2, sizeof(cl_int), &size));
                    clFinish(queue);
                    ...
                    OCL_SAFE_CALL(clSetKernelArg(system->kernel_tanh_deriv_simple, 1, sizeof(cl_mem), &acts_mod.back()));
                    OCL_SAFE_CALL(clSetKernelArg(system->kernel_tanh_deriv_simple, 2, sizeof(cl_int), &size));
                    clFinish(queue);
